...
def do_cpu_line(app, mytime):
    n = {}
    if type(time) != int and mytime < 1:
        print("time参数为int类型且大于0")
        return 0
    for item in devices_info:
        n[item] = []
    for i in range(mytime):
        print(i)
        m = cpu_info_one(devices_info, app)
        for item in m:
            n[item].append(float(m[item][0]))
        time.sleep(1)
    # print(n)
    draw_line_cpu(n)


def do_mem_line(app, mytime):
    n = {}
    if type(time) != int and mytime < 1:
        print("time参数为int类型且大于0")
        return 0
    ...
    for i in range(mytime):
        print(i)
        m = mem_info_one(devices_info, app)
        for item in m:
            n[item].append(round(m[item][0] / 1024, 2))
        time.sleep(1)
    draw_line_mem(n)


def do_fps_line(app, mytime):
    n = {}
    if type(time) != int and mytime < 1:
        print("time参数为int类型且大于0")
        return 0
    ...
    for i in range(mytime):
        print(i)
        m = fps_info_one(devices_info, app)
        if not m:
            print("fps_info_one函数返回为空！")
            return 0
        for item in m:
            n[item][0].append(m[item][0])
            n[item][1].append(round(m[item][1] / m[item][2], 2))
        time.sleep(1)
    draw_line_fps(n)
